...
class Ship:
    ...
    def shoot(self):
        ...
        MiSSILE_GROUP.add(
            Sprite(missile_pos, missile_vel, self.angle, 0,
...
class Sprite:
    ...
    def collide(self, obj):
        dis = dist(obj.pos, self.pos)
        if dis <= self.radius + obj.get_radius():
            return True
...
# mouseclick handlers that reset UI and conditions
...
def draw(canvas):
    ...
    is_ship_collipse = group_collide(ROCK_GROUP, my_ship)
...
def group_group_collide(sets1, sets2):
    ...
    for it1 in set(sets1):
        for it2 in set(sets2):
            if it1.collide(it2):
                sets1.remove(it1)
                sets2.remove(it2)
                score += 1
